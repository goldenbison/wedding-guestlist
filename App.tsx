  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-primary-200">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 px-4 pt-4 pb-2 pointer-events-none">
        <div className="max-w-md mx-auto bg-white/90 backdrop-blur-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] rounded-[2rem] px-5 py-3 flex items-center justify-between pointer-events-auto border border-white/50">
          <div className="flex flex-col justify-center min-h-[2.5rem]">
             <h1 className={`font-black text-slate-900 leading-normal pb-0.5 px-1 ${language === Language.KHMER ? 'text-lg' : 'text-xl'}`}>
               {t.title}
             </h1>
             <div className="flex items-center gap-1.5 mt-0.5 px-1">
                <Heart className="w-3 h-3 text-primary-500 fill-primary-500" />
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none pt-0.5">{t.date}</p>
             </div>
          </div>
          
          <button 
            onClick={handleLanguageToggle}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-50 text-slate-600 font-bold text-sm hover:bg-primary-50 hover:text-primary-600 transition-colors border border-slate-100"
          >
            <Globe className="w-3.5 h-3.5" />
            <span className="pb-0.5 px-1">{language === Language.KHMER ? 'EN' : 'ខ្មែរ'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="pt-28 pb-6 px-6">
        <div className="max-w-md mx-auto text-center">
          <div className="inline-flex flex-col items-center justify-center relative">
             ...
             <span className="relative text-7xl font-black text-slate-900 tracking-tighter leading-none px-2">
              {grandTotal}
             </span>
             <span className="relative text-sm font-black text-primary-500 uppercase tracking-[0.2em] mt-3 leading-loose pb-1 px-1">
               {t.totalGuests}
             </span>
          </div>
          
          <p className="mt-6 text-slate-400 text-sm font-bold uppercase tracking-widest opacity-80 leading-relaxed px-4">
             {t.selectToView}
          </p>
        </div>
      </div>
      ...
      {/* Main Grid */}
      <main className="max-w-md mx-auto px-4 pb-safe-bottom mb-14">
        ...
        <div className="mt-12 text-center pb-8">
            <div className="inline-block h-1.5 w-16 bg-slate-200 rounded-full mb-4"></div>
            <p className="text-slate-300 text-sm font-bold uppercase tracking-widest leading-loose px-1">
               {t.date}
            </p>
        </div>
      </main>
